...
import { FUT_SIGN, roundNumber, getShortFormNumber } from '../../utils/Utils'
...
const OrderBookBox = ({ ticker, onSeverityChange }) => {
    ...
    useEffect(() => {
        console.log(`${ticker} cup rendered.`);
    });
    ...
    const findNewSignificantLevels = (trades, pastOrderBookState, isAsk=true) => {
        ...
        for (const trade of trades) {
            ...
            let level = getDensity(trade, settings);
    ...
    const saveNewTrades = (trades) => {
        ...
        for (const trade of trades) {
            let price = trade[0];
            let density = getDensity(trade, settings);
            if (density === 3) sev++;
    ...
    const getDensityStyle = (data) => {
        ...
        const settings = getSettings(ticker);
        let density = getDensity(data, settings);
            ...
            <div className='ob__container' id={`obContainer_${ticker}`}>
                ...
                <div className='ob__footer'>
                    ...
                    <div style={{width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
                        <b>{getVolume(isDollar, volumePer5M, tickerPrice)}</b>
                    </div>
                </div>
            </div>
        </>
...
const getDensity = (data, settings) => {
